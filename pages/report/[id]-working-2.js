...
{Array.isArray(report?.report?.reportSections) && report.report.reportSections.length > 0 ? (
  <div className="space-y-12"> {/* spacing between main sections */}
        ...
        {Array.isArray(section.subsections) && section.subsections.length > 0 && (
          <div className="space-y-10">
            {section.subsections.map((sub, subIdx) => (
              <div key={subIdx} className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm hover:shadow-md transition">
...
{Array.isArray(report.structuredUpsells) && report.structuredUpsells.length > 0 ? (
  report.structuredUpsells
    ...
    .map((upsell) => {
      ...
      return (
        <div key={upsell?.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
          {/* --- Product Header --- */}
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-lg font-semibold text-gray-800">{displayName}</h4>
            {userHas && (
              <span className="text-green-600 font-medium text-sm">✅ Already in your toolkit</span>
            )}
          </div>
          ...
          <div className="flex justify-between items-center">
            {!userHas && (
              ...
              >
                {upsell?.id === "main_tracker"
                  ? "Start tracking with this system"
                  : `Explore ${displayName}`}
              </a>
            )}
            {userHas && (
